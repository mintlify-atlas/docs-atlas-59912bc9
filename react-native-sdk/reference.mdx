...
These are the props accepted by the `<JitsiMeeting />` component. Only `room` is required; all other props are optional.
...
<ParamField path="onAudioMutedChanged" type="({ muted: boolean }) => void">
...
<ParamField path="onVideoMutedChanged" type="({ muted: boolean }) => void">
...
<ParamField path="onParticipantJoined" type="({ id: string }) => void">
